	  ...
	  if($loggedin == true) {
	    ...
	    //for now, as long at the user is logged in, he/she is eligible to like a question
	  	...
	  	if(empty($errors)) {
    		...
    		$notification_type = "question_like";
    		$notification_text = (($is_like == 0) ? "disliked" : "liked") . " your question";
    		//set the notification
    		$stmt = $dbc->prepare("SELECT is_like, notification_id
    			FROM notifications 
    			WHERE notifier_id = ? AND notified_id = ? AND type = ? AND question_id = ? AND seen = '0' ");
	    	$stmt->bind_param("iisi", $view_user_id, $question_owner_id, $notification_type, $question_id);
    		$stmt->execute();
    		$stmt->store_result();
    		if($stmt->num_rows === 1) {
    			...
    			$stmt = $dbc->prepare("INSERT INTO notifications (notifier_id, notified_id, type, notification_time, question_id, is_like, text) VALUES (?, ?, ?, NOW(), ?, ?, ?) ");
		    	$stmt->bind_param("iisiss", $view_user_id, $question_owner_id, $notification_type, $question_id, $is_like, $notification_text);
	    		$stmt->execute();
    		}
