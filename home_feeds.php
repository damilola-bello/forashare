<?php
    ...
    if($loggedin == true) {
      ...
      if(empty($errors)) {
        ...
        $stmt = $dbc->prepare("SELECT COUNT(DISTINCT q.question_id) 
          FROM question AS q 
          JOIN tag_associations AS ta ON q.question_id = ta.question_id 
          WHERE 
            (ta.tag_id IN (SELECT tag_id FROM tag_following WHERE user_id = ?) 
              OR q.user_id  IN ( SELECT following_id FROM user_following WHERE follower_id = ?))
            AND q.question_heading LIKE CONCAT('%',?,'%') $unanswered_string
        ");
        $stmt->bind_param("iis", $view_user_id, $view_user_id, $search_string);
        $stmt->execute();
        $stmt->store_result();
        $stmt->bind_result($total_questions);
        $stmt->fetch();

        $stmt = $dbc->prepare("SELECT DISTINCT q.question_id, q.question_heading, q.points, q.answers, u.user_id, u.username, TIMESTAMPDIFF(SECOND, q.question_date, NOW()), DATE_FORMAT(q.question_date, '%b %e'), DATE_FORMAT(q.question_date, 'at %h:%i%p'), YEAR(q.question_date), YEAR(NOW()), DAY(q.question_date), DAY(NOW()) 
          FROM user AS u
          JOIN question AS q ON u.user_id = q.user_id
          JOIN tag_associations AS ta ON q.question_id = ta.question_id
          WHERE 
            (ta.tag_id IN (SELECT tag_id FROM tag_following WHERE user_id = ?) 
              OR q.user_id  IN ( SELECT following_id FROM user_following WHERE follower_id = ?))
        ...
        $stmt->bind_param("iisi", $view_user_id, $view_user_id, $search_string, $start);
        $stmt->execute();
        $stmt->store_result();
        $stmt->bind_result($question_id, $question_heading, $points, $answers, $question_user_id, $question_username, $seconds, $question_date_open, $question_date_close, $question_year, $current_year, $question_day, $today);
