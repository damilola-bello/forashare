<?php
    ...
    $q = "";
		...
		$number_pattern = "/^[0-9]+$/";
		//Validation
		if( isset($_GET['start']) && (preg_match($number_pattern, $_GET['start']) && (isset($_GET['tag_id']) && (preg_match($number_pattern, $_GET['tag_id']) ) ) ) ) {
      $start = intval($_GET['start']);
      $tag_id = intval($_GET['tag_id']);

      //check if tag_id is not a country
      ...
      if($temp_count != 1) {
        $errors [] = "Wrong parametersw.";
      }
      unset($temp_count);
    } else {
      $errors [] = "Wrong parametersx.";
    }
    ...
    //check if there is a country parameter
    if( isset($_GET['country_id']) && preg_match($number_pattern, $_GET['country_id']) ) {
      $country_id = intval($_GET['country_id']);
      //check if country_id is a valid country
      $stmt = $dbc->prepare("SELECT COUNT(t.tag_id)
        FROM tag AS t
        JOIN tag_type AS tt ON t.tag_type_id = tt.tag_type_id
        WHERE tt.name = 'forum' AND t.tag_id = ?
      ...
      if($temp_count != 1) {
        ...
        $country_query = "AND EXISTS (SELECT question_id FROM tag_associations WHERE ta.question_id = question_id AND tag_id = $country_id)";      
      }
    }
    ...
    if(empty($errors)) {
      ...
      include('includes/how_long.php');
        ...
        JOIN tag_type AS tt ON t.tag_type_id = tt.tag_type_id
        WHERE t.tag_id = ? $country_query $unanswered AND q.question_heading LIKE CONCAT('%',?,'%')
      ");
			$stmt->bind_param("is", $tag_id, $search_string);
      $stmt->execute();
      $stmt->store_result();
      ...
      if($total_questions > 0) {
        ...
        $stmt->bind_param("isi", $tag_id, $search_string, $start);
        $stmt->execute();
        $stmt->store_result();
